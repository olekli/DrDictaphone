...
class Vad:
  ...
  def onStartVad(self):
    assert self.pass_thru
    self.pass_thru = False
    self.events.active()
    self.events.start_vad()

  def onStopVad(self):
    assert not self.pass_thru
    self.buffer += AudioSegment.silent(duration = 3000 - len(self.buffer))
    self.checkForSpeech(flush = True)
    self.pass_thru = True
    self.buffer = AudioSegment.silent(duration = 2000)
    self.events.idle()
    ...
    if len(self.buffer) >= 3000:
      with tempfile.NamedTemporaryFile(
        ...
        if len(predictions) == 0:
          ...
          if flush:
            self.events.result(self.buffer[predictions[0][0]:predictions[-1][1]])
            self.events.fence()
          else:
